...
const validationSchema = yupObject({
    userName: string().min(3, <Trans i18nKey="message.minChar" values={{value: 3}} defaults={'At Least {{value}} Characters'} />).required(<Trans i18nKey="required" />),
    email: string()
        .label('Email')
        ...
        <View style={{ flex: 0, width: '100%' }}>
            <View style={styles.inputWrapper}>
                <TextInput
                    ...
                    onChange={() => setFieldTouched("userName", true)}
                    // onSubmitEditing={handleChange('userName')}
                    // onBlur={handleBlur}
                    clearButtonMode="while-editing"
                    highlightColor={handleErrorsUI('userName').color()}
                    underlineColorAndroid={handleErrorsUI('userName').color()}
                />
                {
                    handleErrorsUI('userName').text()
                }
            </View>
            <View style={styles.inputWrapper}>
                <TextInput
            ...
            <TouchableOpacity onPress={() => navigation.navigate('Login In')} style={[styles.loginBtn, { opacity: isSubmitting ? 0.5 : 1 }]} disabled={isSubmitting}>
...
const SignupForm = ({navigation, route}) => {
    ...
    const { setItem: setStrgSignedup } = useAsyncStorage("LOGGEDIN");
    ...
    const handleSubmit = async (values, { setSubmitting }) => {
        if (values) {
            ...
            // setTimeout(() => {
                await setStrgSignedup(JSON.stringify(true));
                setSubmitting(false);
